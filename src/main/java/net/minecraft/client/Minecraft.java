...
//        this.ingameGUI = new GuiIngame(this);
    ...
    private void rightClickMouse()
    {
        if (!this.playerController.func_181040_m())
        {
            ...
            {
                if(thePlayer.inventory.getCurrentItem() != null && Paradise.INSTANCE.toolBinds.get(thePlayer.inventory.getCurrentItem().getItem()) != null) {
                    thePlayer.sendChatMessage(Paradise.INSTANCE.toolBinds.get(thePlayer.inventory.getCurrentItem().getItem()));
                }
    ...
    public void dispatchKeypresses()
    {
        ...
        if (i != 0 && !Keyboard.isRepeatEvent())
        {
            if (!(this.currentScreen instanceof GuiControls) || ((GuiControls)this.currentScreen).time <= getSystemTime() - 20L)
            {
                if (Keyboard.getEventKeyState())
                {
                    if(Paradise.INSTANCE.keyToolBinds.containsKey(i) && thePlayer != null && this.currentScreen == null) {
                        thePlayer.sendChatMessage(Paradise.INSTANCE.keyToolBinds.get(i));
                    }
